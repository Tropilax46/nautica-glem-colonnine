...
export default function Layout({ children }: { children: ReactNode }) {
  ...
  const linkCls = (active: boolean) => `block rounded px-3 py-2 text-sm ${active ? "bg-glem-500" : "hover:bg-glem-500/40"}`;
  ...
  return (
    <div className="min-h-screen lg:flex">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 flex w-64 flex-col overflow-y-auto bg-glem-700 p-6 text-white transition-transform lg:static lg:z-auto lg:w-60 lg:translate-x-0 ${drawer ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <nav className="flex-1 space-y-1">
          {NAV.map((it) =>
            it.children ? (
              <div key={it.label}>
                <button onClick={() => toggle(it.label)} className="flex w-full items-center justify-between rounded px-3 py-2 text-sm hover:bg-glem-500/40">
                  <span>{it.label}</span><span className="text-xs opacity-70">{open[it.label] ? "▾" : "▸"}</span>
                </button>
                {open[it.label] && (
                  <div className="mt-1 space-y-1 border-l border-white/15 pl-3">
                    {it.children.map((c) => (<Link key={c.href} href={c.href} onClick={() => setDrawer(false)} className={linkCls(path === c.href)}>{c.label}</Link>))}
                  </div>
                )}
              </div>
            ) : (
              <Link key={it.href} href={it.href!} onClick={() => setDrawer(false)} className={linkCls(path === it.href)}>{it.label}</Link>
            )
          )}
        </nav>
